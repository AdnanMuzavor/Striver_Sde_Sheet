...
EasyAccuracy: 42.73%Submissions: 134K+Points: 2
Lamp
...
class Solution
{
    ...
    long long minCost(long long arr[], long long n) {
        // Your code here
        long long cost=0;
        //Min  heap to keep minimum costs aways at top
        ...
        while(!min_heap.empty()){
            ...
            //Only if heapsize is > 2 means there are ropes left to be join
            //Henc eextract and join them up
            if(min_heap.size()>=2){
                int n=2;
                while(n--){
                new_cost+=min_heap.top();
                min_heap.pop();
                }
            }
            else{
            ...
            //Add the enw minimum cost of joining ropes to current cost
            cost+=new_cost;
           // cout<<"new cost: "<<new_cost<<" cost: "<<cost<<endl;
            //Push new cost into min_hepa so that it can re-heapify and give us next minimum cost
            min_heap.push(new_cost);
        }
        return cost;
    }
};
